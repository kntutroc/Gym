...
class PilatesRoutineActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_pilates_routine)
        ...
        fetchBodyWeightExercises()
    }

    private fun fetchBodyWeightExercises() {
        val client = OkHttpClient()

        val request = Request.Builder()
            .url("https://exercisedb.p.rapidapi.com/exercises/equipment/body%20weight?limit=10")
            .get()
        ...
        client.newCall(request).enqueue(object : Callback {
            override fun onFailure(call: Call, e: IOException) {
                Log.e("PilatesRoutineActivity", "Error fetching exercise", e)
            }

            override fun onResponse(call: Call, response: Response) {
                if (!response.isSuccessful) {
                    Log.e("PilatesRoutineActivity", "Unexpected response code: ${response.code}")
                    return
                }
